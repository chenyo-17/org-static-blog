...
* Event-driven programming
   ...
   - Threads: a lightweight process; has own stack, CPU registers, status; managed by OS scheduler, e.g., swap on and off the core.
     - States: block, ready, running
   - Block syscalls: every thread runs for a time slice before switched by OS, or give up the slice early if it is blocked by e.g., ~read~, ~waitpid~, ~cv.wait~, ~lock~, ~sleep~.
   - Switching threads are expensive:
     - Context switching cost: switch virtual address space, restore register and cache, etc.
     - Memory overhead: each thread consumes stack memory.
   - The thread could have done other work when it is blocked on one request.
   - **Non-blocking I/O**: One thread handles concurrent I/O with [[https://en.wikipedia.org/wiki/Epoll][Epoll]], which repeatedly notifies what file descriptors are ready for I/O.
     - Key problem: state management.
...
*** Executor
- An executor loops over futures that can currently make progress and call ~poll~ on them; or go to sleep until some future calls ~wake~.
- A popular executor is Tokio; it allows multiple executors to execute futures in parallel.
  - Need to protect shared data with synchronization primitives acorss futures.
- Futures cannot block, e.g., call ~lock~, otherwise the executor cannot continue other futures.
  - Tokio provides a non-blocking versions for synchronization primitives.

*** Tokio
- Convert blocking functions into async versions.
#+BEGIN_SRC rust
use tokio::io::{AsyncReadExt, AsyncWriteExt};
use tokio::net::TcpListener;  // should not block in async code

// futures do not do anything until an executor executes them
// the macro generates a sync main which creates a runtime executor 
// and submits the returned future to the executor
#[tokio::main]
async fn main() {
let listener = TcpListener::bind("127.0.0.1:8080").await.unwrap();
loop {
    let (mut socket, _) = listener.accept().await.unwrap();
    tokio::spawn(async move {  // an async block
        let mut buf = [0; 1024];
...
- Normal (synchronous) code stores variables on the stack.
  - When calling another function, the inner function gets a new stack frame and CPU remembers the return address.
  - When the function returns, the stack frame is popped and CPU jumps back to the return address.
- Async functions may return multiple times at each ~.await~, everytime it returns, the stack frame is popped, so the async function needs to maintain its state in its returned future object.
- The compiler transforms the async function into a state enum that implements ~Future~ and an implementation of ~poll()~.
#+BEGIN_SRC rust
enum AddToInboxState {
    ...
    WaitingLoadMessage {
        recipient_id: u64, // the data is to be used in the next state
        state: LoadMessageFuture,
    },
    WaitingGetRecipient {
        message: Message,
        state: GetRecipientFuture,
    },
    WaitingAddToInbox {
...
// this is only concept code, not real implementation
fn poll() {
    match self.state {
        NotYetStarted(email_id, recipient_id) => {
            let next_future = load_message(email_id);
            self.state = AddToInboxState::WaitingLoadMessage {
                recipient_id: *recipient_id,
                state: next_future,
            };
        }
        WaitingLoadMessage(recipient_id, state) => {
            match state.poll() {
                Ready(message) => {
        ...
        WaitingGetRecipient(message, state) => {
            match state.poll() {
                Ready(recipient) => {
...
- All compiled futures are registered in the executor, the executor works as a scheduler and polls each future when it wakes up.
- The executor does not care each future's internal state, this is maintained in the future context.
- The executor polls ~main~ future first and run until it reaches ~.await~.
- When an async function polls another async function, if the inner future returns ~Pending~, the outer future also returns ~Pending~; when the inner future is ready, it wakes the whole task in the executor, and the executor polls outer future again.
- Sync functions consume stack, async functions consume heap as each future is live on heap.
- Usage: I/O bound and highly concurrent, e.g., thousand network connections, DB queries.
